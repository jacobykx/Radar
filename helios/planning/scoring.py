...
result stays on the 1-5 scale whatever the weights are.

Rule 3: a user may override the computed priority, but the computed value is never
overwritten — both are kept, and an override without a rationale is refused.

Rule 5: mandated reviews are not driver-scored at all. They are pinned by an external
obligation, so a priority for them would be meaningless.
"""

from __future__ import annotations

from .constants import BAND_FLOOR, Band
from .errors import RationaleRequired
from .types import Review, Scores, Weights
...
def computed_priority(scores: Scores, weights: Weights) -> float:
    ...
    With every weight at zero the numerator is zero too, so the fallback denominator of 1
...
def effective_priority(review: Review, weights: Weights) -> float | None:
    """The priority in force: the override if set, else the computed value."""
    if review.mandated:
        return None
...
def band_of(review: Review, weights: Weights) -> Band:
...
def sort_key(review: Review, weights: Weights) -> tuple[int, float]:
