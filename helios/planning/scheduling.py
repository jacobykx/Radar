...
remaining FTE, mandated first and then by priority. A function's quarterly FTE is never
exceeded; anything that cannot fit is left unscheduled and flagged rather than squeezed in.

Mandated reviews that already hold a quarter — derived from the regulatory go-live date —
keep it and commit their capacity before anything else is placed. They are pinned by an
external obligation, so the scheduler works around them rather than moving them.
"""
...
def _pinned(review: Review) -> bool:
...
def waterfall(
    ...
    Pure: nothing is mutated. The caller applies `result.quarters`.
    """
    ...
    for capacity in capacities:
        ...
        load: dict[Quarter, int] = dict.fromkeys(QUARTERS, 0)
